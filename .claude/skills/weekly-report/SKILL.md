...
The script does not print the report itself — only where it saved the two output files (a `.json` report and a `.transcript.jsonl` file), both under `usage_reports/`. Show that output verbatim to the student. Do not guess what needs to be submitted — that's specified in the assignment, not by this skill. Do not open or paste the `.json` file's contents into the conversation yourself unless the student explicitly asks to see it.
