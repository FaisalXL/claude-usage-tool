...
The script does not print the report itself — only where it saved the two output files (a `.json` report and a `.transcript.jsonl` file), both under `usage_reports/`. Show that output verbatim to the student, and tell them both files need to be submitted. Do not open or paste the `.json` file's contents into the conversation yourself unless the student explicitly asks to see it.
